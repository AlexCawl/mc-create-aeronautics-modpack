...
For a pinned release, run the `Release mrpack` workflow manually and set `release_tag` to a new SemVer-style tag with a `v` prefix, for example `v0.1.0`. Manual releases are immutable in this repository policy: an existing tag is not overwritten.
